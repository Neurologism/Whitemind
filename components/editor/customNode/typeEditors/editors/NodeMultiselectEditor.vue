...
function onItemClick(option: string) {
  if (
    nodesData.value!.data[props.paramName].some((e: string) => e === option)
  ) {
    nodesData.value!.data[props.paramName] = nodesData.value!.data[
      props.paramName
    ].filter((e: string) => e !== option);
  } else {
...
<template>
  ...
  >
    ...
    >
      ...
      <UIcon
        v-if="nodesData?.data[paramName].some((e: string) => e === option)"
        class="text-base"
        name="mdi-check"
      />
    </div>
