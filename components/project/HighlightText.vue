...
function splitAtMultiple(text: string, nodes: NodeDefinition[]) {
  ...
  const regex = new RegExp(`(${justNames.join('|')})(?![a-zA-Z])`, 'gi');
...
const highlightedParts = computed(() => {
  ...
  const split = splitAtMultiple(
    props.text + ' lorem ips datasets',
    projectStore.editorConfig.allNodes
  );
...
<template>
  <span
    ...
    :key="index"
    class="font-mono text-sm leading-none p-0 m-0"
    :class="{
