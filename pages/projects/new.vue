...
<template>
  <div class="flex w-screen justify-center flex-row conte">
    <div class="md:basis-2/3 lg:basis-2/5 sm:basis-full basis-full dark:divide-slate-700 mx-5 p-5">
      <h1 class="sm:text-3xl text-2xl">Create a new Project</h1>
      <span class="text-gray-500">Id velit magnam ipsam nostrum pariatur facilis ea ab. Quibusdam non minima corporis nemo voluptatibus. </span>
      <div class="mt-1 mb-1">
        <UDivider />
      </div>
      <div class="input-tile">
        <HintBox>
          <UInput v-model="projectName" label="Project Name" placeholder="Enter the name of your project" />
          <template #hint>
            <UAlert
              icon="mdi-info"
              color="orange"
      ...
      <div class="input-tile">
        <UTabs :items="tabValues" @change="(index) => projectVisible = index == 0"/>
      </div>
      <div class="input-tile">
        <UDivider />
      </div>
      <div class="input-tile">
        <UButton
          block
          :disabled="!projectName || !projectDescription || isProcessingCreation"
          color="primary"
          @click="createProject"
